...
// 特征序列
struct TraitsDataListModel {
    let data = Observable.just(["Single", "AsSingle", "Completable", "Maybe", "AsMaybe"])
}

class TraitsViewController: UIViewController {
    let disposeBag = DisposeBag()
    let dataList = TraitsDataListModel()
        ...
        cacheLocally()
            .subscribe { completable in
                ...
                case .completed:
                    log.debug("保存成功!")
                case let .error(error):
                    log.debug("保存失败: \(error.localizedDescription)")
                }
            }
            .disposed(by: disposeBag)
        ...
        generateString()
            .subscribe { maybe in
                ...
                case let .success(element):
                    log.debug("执行完毕，并获得元素：\(element)")
                case .completed:
                    log.debug("执行完毕，且没有任何元素。")
                case let .error(error):
                    log.debug("执行失败: \(error.localizedDescription)")
                }
            }
            .disposed(by: disposeBag)
