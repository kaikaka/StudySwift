...
//        假设我们想实现点击按钮后，弹出一个消息提示框
        clickBtn = UIButton(type: .system)
        clickBtn.frame = CGRect(x: 100, y: 100, width: 200, height: 200)
        view.addSubview(clickBtn)
        clickBtn.backgroundColor = .red
...
//        绑定背景图片
        timer.map({
            ...
            .bind(to: clickBtn.rx.backgroundImage())
            .disposed(by: disposeBag)
    }
...
//    将数字转成对应的富文本
    ...
    func moreSelected() {
        ...
        for button in btns {
            log.debug("----------")
            selectBtn.map {
                log.debug("\(button.titleLabel?.text),\($0.titleLabel?.text)")
                return $0 == button
