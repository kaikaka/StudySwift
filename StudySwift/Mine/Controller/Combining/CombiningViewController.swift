...
//结合操作符
struct CombiningDataListModel {
    let data = Observable.just(["StartWith","Merge","Zip","CombineLatest",
                                "WithLatestFrom","SwitchLatest"])
}

class CombiningViewController: UIViewController {
    let disposeBag = DisposeBag()
    let dataList = CombiningDataListModel()
...
//        比如本来监听的 subject1，我可以通过更改 BehaviorRelay 里面的 value 更换事件源。变成监听 subject2。
