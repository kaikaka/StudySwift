...
class TableViewController<RVM: RefreshViewModel>: BaseViewController<RVM> {
    ...
    // MARK: - 绑定无数据状态 Footer是否显示
    
    func bindEmptyDataSetView() {
        if self.tableView.isTotalDataEmpty {
            self.tableView.refreshFooter?.isHidden = true
            self.tableView.refreshFooter?.resetNoMoreData()
        } else {
            self.tableView.refreshFooter?.isHidden = false
        }
    }
